...
Safe rules require more significant modification, as EDPP no longer applies. In its place, we use Slores \citep[sparse logistic regression screening rule,][]{wang2014safe}. It was proposed as a one-step rule but can be extended to an adaptive rule with minimal changes. %Let $\Tilde{y}=2y-1\in\{-1,1\}^n$, $r(\lambda_k)=y-\pi(\hat{\beta}(\lambda_k))$.
...
Then $\hat{\beta}_j(\lambda_{k+b})=0$ if
  ...
  \end{equation}
  \pb{This appears to be missing something. This is not a true/false expression, it's just a number.}
\end{theorem}
